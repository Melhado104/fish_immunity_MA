...
fitstats(mod_lnRR.assay_id.study_id, mod_lnRR.assay_id.study_id.species.phylogeny)
...
```{r}
## *Figure 2*
#Impacts of pharmaceutical exposure on fish immune response. The model estimates the average effects of pharmaceutical exposure on immune responses in fish. (A) shows the mean difference between control and treatment groups on a logarithmic scale, where negative values indicate a weak immune response.
...
intercept_only.lnRR_orchard <- orchaRd::orchard_plot(lnRR_intercept_only,
  ...
  theme(
    ...
    panel.grid.major = element_blank(),# removes major gridlines (optional redundancy)
    panel.grid.minor = element_blank() # removes minor gridlines (optional redundancy)
  )
...
```{r}
immune_category_high_mod_orchard.lnRR <- orchaRd::orchard_plot(lnRR_mod_immune_category_high,
                                                        group = "study_id", 
                                                        xlab = "lnRR",
...
```{r}
# Figure 3
# Step 1: Create a data frame with one row per immune_category_high
trait_levels_rr <- data.frame(immune_category_high = sort(unique(data$immune_category_high)))

# Step 2: Create the model matrix
X_rr <- model.matrix(~ immune_category_high - 1, data = trait_levels_rr)

# Step 3: Get predictions from the model
preds_rr <- predict(lnRR_mod_immune_category_high, newmods = X_rr, level = 95)

# Step 4: Build the plot data frame
immune_plot_data_rr <- data.frame(
  immune_category_high = trait_levels_rr$immune_category_high,
...
# Step 6: Plot
ggplot(immune_plot_data_rr, aes(x = estimate, y = immune_category_high)) +
